...
def lane_follow_step(world, vehicle, lookahead_m, steer_gain,
                     ...
                     use_abs=False,           # software ABS: pulse brake to prevent wheel lock
                     ):
    """
    ...
    # -------------------------------------------------
    # Software ABS (Anti-lock Braking System)
    # -------------------------------------------------
    # CARLA's PhysX tires transition from rolling to kinetic (sliding) friction
    # when brake force locks the wheels. Kinetic friction < static friction, so
    # a fully locked wheel actually produces LESS stopping force than a wheel at
    # the slip threshold. Real ABS pulses brake pressure to keep wheels just
    # below lock. We approximate this by:
    #   1. Monitoring actual deceleration each tick (speed_mps delta / dt).
    #   2. If brake is above the lock threshold AND decel is below what rolling
    #      friction should produce, we release the brake briefly (ABS_RELEASE)
    #      to let wheels spin up, then allow normal ramp to resume next tick.
    #
    # Tuning constants:
    #   ABS_ENGAGE_BRAKE  : brake command level above which lock is possible
    #   ABS_EXPECTED_DECEL: m/s² we expect from near-full braking (dry road ~8-9 m/s²)
    #   ABS_RELEASE       : brake level to pulse down to (lets wheels recover)
    #   ABS_MIN_SPEED     : don't run ABS below this (irrelevant at near-stop)
    if use_abs:
        ABS_ENGAGE_BRAKE   = 0.75   # start checking above this brake level
        ABS_EXPECTED_DECEL = 7.0    # m/s² — below this at high brake = likely locked
        ABS_RELEASE        = 0.55   # pulse down to this to recover wheel spin
        ABS_MIN_SPEED      = 1.5    # m/s — don't pulse at near-stop

        prev_speed_abs = speed_state.get("abs_prev_speed_mps", speed_mps)
        actual_decel   = (prev_speed_abs - speed_mps) / fixed_dt  # positive = slowing
        speed_state["abs_prev_speed_mps"] = speed_mps

        if (b_new > ABS_ENGAGE_BRAKE
                and speed_mps > ABS_MIN_SPEED
                and actual_decel < ABS_EXPECTED_DECEL):
            # Wheel lock detected — release briefly
            b_new = ABS_RELEASE
