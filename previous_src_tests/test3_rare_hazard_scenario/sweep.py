...
def make_grid_sweep(
    ...
    **fixed_kwargs,
    ...
    configs = []
    for (mph, enc, side, cross, wspeed, weather, ttc, profile, headway, abs_on) in combos:
        cfg = ScenarioConfig(
            target_mph=mph,
...
def _write_summary_csv(output_dir: str, configs: List[ScenarioConfig], results: list):
    ...
    fieldnames = [
        ...
        "braking_ramp_up_per_s", "brake_headway_s", "brake_profile", "use_abs",
...
def _print_sweep_summary(results: list, configs: Optional[List] = None):
    ...
    # Split by use_abs if both values present
    if configs:
        abs_vals = sorted(
            v for v in set(getattr(cfg, "use_abs", None) for cfg in configs)
            if v is not None
        )
        if len(abs_vals) > 1:
            print("\n--- Breakdown by ABS setting ---")
            for abs_on in abs_vals:
                pairs = [
                    (cfg, r) for cfg, r in all_pairs
                    if getattr(cfg, "use_abs", None) == abs_on
                ]
                label = "ABS ON  (wheel-lock prevention active)" if abs_on else "ABS OFF (baseline, wheels can lock)"
                _section(f"  {label}", pairs)
    print()
...
if __name__ == "__main__":
    ...
    SWEEP_CONFIGS = make_grid_sweep(
        ...
        brake_headway_s=[2.5],
        # ABS sweep: off vs on.
        # Tests whether software ABS (wheel-lock prevention) reduces collisions,
        # especially for step_constant which applies full brake instantly.
        # Expected: ABS=True should help step_constant most — it locks wheels
        # hardest, losing kinetic vs static friction advantage.
        use_abs=[False, True],
        sim_seconds=30.0,  # hard ceiling only — runs end dynamically when ped finishes crossing + 3s settle
        # Total: 3 speeds x 2 distances x 2 walker_cross x 2 ttc x 4 profiles x 2 abs = 192 runs
    )
