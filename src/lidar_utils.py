...
def lidar_min_distance_along_transition_corridor(
    lidar_actor,
    lidar_data,
    route_points_world,
    ego_loc,
    *,
    ...
    """Measure a swept transition corridor between the ego's actual lateral
    offset and the commanded target offset.

    Unlike the constant-offset corridors in
    ``lidar_min_distances_along_route_corridors``, this corridor's lateral
    center blends from ``start_offset_m`` at the ego's current position to
    ``target_offset_m`` over the next ``blend_distance_m`` meters along the
    route (smoothstep-eased, then held at ``target_offset_m`` beyond that).
    This approximates the vehicle's actual swept path during a lane-change
    style maneuver, rather than a parallel lane offset that the ego is not
    yet on.

    This remains a geometric approximation of a future path, not a
    validated safety envelope -- it is observational only and must not gate
    braking or be treated as proof a maneuver is safe.
    """
    if (
        lidar_data is None
        or lidar_actor is None
    ...
    blend_span = max(1e-6, blend_distance_m)

    def corridor_center_offset(along_route_m):
        fraction = _smoothstep01(along_route_m / blend_span)
        return start_offset_m + (target_offset_m - start_offset_m) * fraction

    lidar_transform = lidar_actor.get_transform()
    lidar_origin = lidar_transform.location

    shifted_points = []
    for (x, y), s_here, route_index in zip(route_xy, route_s, route_indices):
        right_x, right_y = route_unit_right_xy(route_points_world, route_index)
        center_offset = corridor_center_offset(s_here)
        shifted_points.append(
            carla.Location(
                x=x + right_x * center_offset,
                y=y + right_y * center_offset,
                z=lidar_origin.z - 1.5,
            )
        )
    ...
    for lidar_point in lidar_data:
        ...
        corridor_center = corridor_center_offset(best_along_route_m)
        if (best_signed_lateral_m - corridor_center) ** 2 > half_width_sq:
            continue
        ...
        if min_d is None or physical_distance_m < min_d:
            min_d = physical_distance_m
